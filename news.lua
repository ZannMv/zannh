...
local ZannHubConfig = Window.ConfigManager:CreateConfig("zannhub")
...
local function Reg(id, element)
    ZannHubConfig:Register(id, element)
...
local BaseFolder = "WindUI/" .. (Window.Folder or "ZannHub") .. "/config/"
...
do
    ...
    _G.ZannHub_AutoAcceptTradeEnabled = false 
    ...
    if PromptController and PromptController.FirePrompt and Promise then
        ...
        PromptController.FirePrompt = function(self, promptText, ...)
            ...
            if _G.ZannHub_AutoAcceptTradeEnabled and type(promptText) == "string" and promptText:find("Accept") and promptText:find("from:") then
        ...
        warn("[ZannHub] Gagal memuat PromptController/Promise untuk Auto Accept Trade.")
    end
end
        ...
        if data then
            ...
            if targetPlayer.Character then
                for _, v in ipairs(targetPlayer.Character:GetChildren()) do
                    if v.Name == "ZannHubESP" and v:IsA("BillboardGui") then pcall(function() v:Destroy() end) end
                end
            end
        end
    end
...
do
    ...
    _G.ZannHub_BlatantActive = false
    ...
    task.spawn(function()
        ...
        if S1 and FishingController then
            ...
            FishingController.RequestChargeFishingRod = function(...)
                if _G.ZannHub_BlatantActive then return end 
                return Old_Charge(...)
            end
            FishingController.SendFishingRequestToServer = function(...)
                if _G.ZannHub_BlatantActive then return false, "Blocked by ZannHub" end
                return Old_Cast(...)
            end
        end
    end)
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if _G.ZannHub_BlatantActive and not checkcaller() then
        ...
        if active then
            task.spawn(function()
                ...
                local InactiveColor = ColorSequence.new({
                    ColorSequenceKeypoint.new(0, Color3.fromHex("4FC3F7")), 
                    ColorSequenceKeypoint.new(1, Color3.fromHex("2196F3"))
                })

                while _G.ZannHub_BlatantActive do
    ...
    local togblat = Reg("blatantt",blatant:Toggle({
        ...
        Callback = function(state)
            ...
            _G.ZannHub_BlatantActive = state
...
local function GetItemsToFavorite()
    ...
    local accept = trade:Toggle({
        ...
        Value = false,
        Callback = function(state)
            _G.ZannHub_AutoAcceptTradeEnabled = state
...
do
    ...
    local function CreateKaitunUI()
        local old = game.CoreGui:FindFirstChild("ZannHubKaitunStats")
        if old then old:Destroy() end
        local sg = Instance.new("ScreenGui")
        sg.Name = "ZannHubKaitunStats"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                if not _G.BlackScreenGUI then
                    ...
                    _G.BlackScreenGUI.Name = "ZannHub_BlackBackground"
        ...
        Callback = function(state)
            ...
            if state then
                ...
                for _, gui in ipairs(PlayerGui:GetChildren()) do
                    if gui:IsA("ScreenGui") and gui.Name ~= "WindUI" and gui.Name ~= "CustomFloatingIcon_ZannHub" then
...
do
    ...
    local WEBHOOK_URL = ""
    local WEBHOOK_USERNAME = "ZannHub Notify" 
    ...
    local GLOBAL_WEBHOOK_USERNAME = "ZannHub | Community"
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            ...
            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("<:TEXTURENOBG:1438662703722790992> ZannHub | Webhook\n\n<a:ChipiChapa:1438661193857503304> New Fish Caught! (%s)", fishName)
                ...
                local embed = {
                    ...
                    footer = {
                        text = string.format("ZannHub Webhook • Total Caught: %s • %s", caughtDisplay, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
            ...
            if isGlobalTarget and GLOBAL_WEBHOOK_URL ~= "" then 
                ...
                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> ZannHub | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", fishName)
                ...
                local globalEmbed = {
                    ...
                    thumbnail = { url = imageUrl },
                    footer = {
                        text = string.format("ZannHub Community| Player: %s | %s", censoredPlayerName, os.date("%Y-%m-%d %H:%M:%S"))
                    }
                }
        ...
        if not success then
            warn("[ZannHub Webhook] Error processing fish data:", results)
        end
    end
...
do
    ...
    about:Section({
        Title = "Join Discord Server ZannHub",
        TextSize = 20,
    })
        ...
        Buttons = {
            {
                ...
                Icon = "link",
                Callback = function()
                    setclipboard("https://discord.gg/racfEk6NGQ")
                    WindUI:Notify({
                        Title = "Link Disalin!",
                        Content = "Link Discord ZannHub berhasil disalin.",
                        Duration = 3,
                        Icon = "copy",
                    })
                end,
...
local function CreateFloatingIcon()
    local existingGui = PlayerGui:FindFirstChild("CustomFloatingIcon_ZannHub")
    if existingGui then existingGui:Destroy() end

    local FloatingIconGui = Instance.new("ScreenGui")
    FloatingIconGui.Name = "CustomFloatingIcon_ZannHub"
    ...
    FrameStroke.Color = Color3.fromHex("2196F3")
...
WindUI:Notify({ Title = "ZannHub Was Loaded", Content = "Press [F] to open/close the menu", Duration = 5, Icon = "info" })
-- [[ AUTO LOAD & SAVE LOOP ]]
task.spawn(function()
    ...
    while true do
         task.wait(10)
         pcall(function() ZannHubConfig:Save() end)
    end
end)
InitializeIcon()
